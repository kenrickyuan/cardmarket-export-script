...
import { MOXFIELD_CONDITIONS, MOXFIELD_LANGUAGES, REGEX_PATTERNS, CARDMARKET_SELECTORS } from '../constants';
import { SET_NAME_TO_CODE_MAPPING } from '../data/set-mappings';
import type { MoxfieldExportData, CardMarketPageType } from '../types/cardmarket';
...
const extractArticleData = (row: Element): MoxfieldExportData | null => {
  try {
    ...
    const condition = MOXFIELD_CONDITIONS[conditionId as keyof typeof MOXFIELD_CONDITIONS] || 'NM';
    const language = MOXFIELD_LANGUAGES[languageId as keyof typeof MOXFIELD_LANGUAGES] || 'en';
...
const createExportButtons = (): void => {
  ...
  if (collapsibleMemo) {
    const memoMoxfieldButton = moxfieldButton.cloneNode(true) as HTMLInputElement;
