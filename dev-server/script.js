...
// Build time: 2025-06-07T10:56:25.433Z
...
(function () {
    ...
    // MTG Set Name to Code Mappings for Moxfield Export
    ...
    const SET_NAME_TO_CODE_MAPPING = {
    ...
    const extractArticleData = (row) => {
        try {
            const articleElement = row;
            ...
            const cleanName = match ? match[1].trim() : dataset.name.trim();
            ...
            const condition = MOXFIELD_CONDITIONS[conditionId] || 'NM';
            const language = MOXFIELD_LANGUAGES[languageId] || 'en';
            // Check for foil
            const foilElement = row.querySelector('div.col-extras span[aria-label="Foil"]');
            ...
            devError(error instanceof Error ? error : new Error(String(error)), 'extractArticleData');
            return null;
        }
    };
    // Convert set name to set code for Moxfield
    const getSetCode = (expansionName) => {
        if (!expansionName)
            return '';
    ...
    const generateMoxfieldCSV = (articles) => {
    ...
    const downloadCSV = (csvContent, filename) => {
        ...
        const exportedFilename = filename.endsWith('.csv') ? filename : `${filename}.csv`;
        // Modern browsers
        ...
        if (link.download !== undefined) {
            ...
            devError(new Error('CSV download not supported in this browser'), 'downloadCSV');
        }
    };
    // Export articles to clipboard as text
    const exportToClipboard = (articles) => {
    ...
    const exportMoxfieldCSV = () => {
        try {
            ...
            devError(error instanceof Error ? error : new Error(String(error)), 'exportMoxfieldCSV');
        }
    };
    ...
    const exportToText = () => {
        try {
            ...
            exportToClipboard(articles).then(() => {
            ...
            devError(error instanceof Error ? error : new Error(String(error)), 'exportToText');
        }
    };
    // Create export buttons and add to page
    const createExportButtons = () => {
        const collapsibleExport = document.querySelector(CARDMARKET_SELECTORS.collapsibleExport);
        const collapsibleMemo = document.querySelector(CARDMARKET_SELECTORS.collapsibleMemo);
        if (!collapsibleExport) {
            devLog('Export container not found');
            return;
        }
        // Update existing text
        const exportDescription = collapsibleExport.querySelector('p.font-italic.small');
        if (exportDescription) {
            exportDescription.textContent = 'Click here to export your articles to a CSV document or copy them to clipboard.';
        }
        ...
        if (collapsibleMemo) {
            const memoMoxfieldButton = moxfieldButton.cloneNode(true);
            memoMoxfieldButton.addEventListener('click', exportMoxfieldCSV);
            collapsibleMemo.appendChild(memoMoxfieldButton);
        }
        devLog('Export buttons created and added to page');
    };
    // Initialize export functionality
    const initializeExport = (pageType) => {
        if (!pageType.isOrdersPage) {
            devLog('Skipping export functionality - not on orders page');
            return;
        }
        try {
            ...
            devError(error instanceof Error ? error : new Error(String(error)), 'initializeExport');
        }
    };
